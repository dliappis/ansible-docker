...
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt-get -y update && \
    apt-get -y install build-essential ca-certificates curl expect jq less libffi-dev libssl-dev make openssh-client python python-dev python-setuptools unzip wget && \
    ...
    pip install boto boto3 pywinrm requests xmltodict && \
